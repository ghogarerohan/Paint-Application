...
export default class Fill {
    ...
    constructor(canvas: HTMLCanvasElement, point: Point , color: any){
    ...
    floodFill(point: Point,targetColor: number[], fillColor: number[]){
    ...
    getPixel(point: Point){
        ...
        if(point.x < 0 || point.y <0 || point.x > this.imageData.width || point.y >this.imageData.height){
            ...
            const offset:number = (point.y *this.imageData.width + point.x)*4;
            //console.log(offset);
     ...
     setPixel(point: Point, fillColor: number[]){
        const offset:number = (point.y *this.imageData.width + point.x)*4;
     ...
     colorsMatch(color1: number[],color2: number[]){
